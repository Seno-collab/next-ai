import { changePassword, requireAuthContext } from "@/features/auth/server/authService";
...
async function handleChangePassword(request: NextRequest) {
  ...
  try {
    ...
    if (AUTH_BASE_URL && AUTH_BASE_URL !== origin) {
      ...
      const response = await fetch(`${AUTH_BASE_URL}/api/auth/change-password`, {
        method: "PATCH",
    ...
    return NextResponse.json({ user, message: t("auth.success.changePassword") });
  } catch (error) {
    return NextResponse.json(
      {
        message: error instanceof Error ? t(error.message) : t("auth.errors.changePasswordFailed"),
      },
      { status: 400 },
    );
  }
